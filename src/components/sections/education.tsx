        ...
        >
          ...
          <div className="max-w-4xl mx-auto space-y-12">
            ...
            <motion.div
              ...
              className="glass p-8 rounded-3xl cursor-default"
            >
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 space-y-3">
                  ...
                  <p className="text-lg text-indigo-500">Instituto Federal do Espírito Santo</p>
                  ...
                  <div className="pt-4 space-y-2">
                    <p className="text-foreground font-semibold">Projeto de Conclusão de Curso (TCC):</p>
                    ...
                    <motion.a
                      ...
                      whileTap={{ scale: 0.95 }}
                      className="inline-flex items-center gap-2 mt-3 px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white rounded-lg font-medium hover:shadow-lg hover:shadow-indigo-500/50 transition-all"
                    >
                  ...
                  <div className="pt-6 space-y-3 border-t border-foreground/10 mt-6">
                    <p className="text-foreground font-semibold">Tecnologias Utilizadas no TCC:</p>
                    <div className="flex flex-wrap gap-2">
                      <span className="px-3 py-1 bg-indigo-500/10 text-indigo-400 rounded-full text-sm font-medium">Python</span>
                      <span className="px-3 py-1 bg-purple-500/10 text-purple-400 rounded-full text-sm font-medium">OpenCV</span>
                      <span className="px-3 py-1 bg-pink-500/10 text-pink-400 rounded-full text-sm font-medium">NumPy</span>
                      <span className="px-3 py-1 bg-indigo-500/10 text-indigo-400 rounded-full text-sm font-medium">Shapely</span>
                      <span className="px-3 py-1 bg-purple-500/10 text-purple-400 rounded-full text-sm font-medium">Visão Computacional</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="text-2xl font-bold font-grotesk">Certificações</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {certifications.map((cert, index) => (
                  <motion.div
                    ...
                    className="glass p-6 rounded-2xl hover:bg-foreground/5 transition-colors cursor-default"
                  >
                    <h4 className="font-bold text-lg mb-2">{cert.name}</h4>
                    <p className="text-sm text-muted-foreground">{cert.institution}</p>
                    <p className="text-sm text-indigo-500 mt-2">{cert.year}</p>
                  </motion.div>
                ))}
              </div>
            </div>
                ...
                {disciplines.map((discipline, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className="glass p-6 rounded-2xl hover:bg-foreground/5 transition-all cursor-default group"
                    >
                      <div className="flex items-center gap-3 mb-4">
                        <motion.div 
                          className="p-2.5 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 rounded-xl group-hover:from-indigo-500/30 group-hover:to-purple-500/30 transition-all"
                      ...
                      <ul className="space-y-2.5">
                        {discipline.items.map((item) => (
                          <li key={item} className="flex items-start gap-2.5 text-sm text-muted-foreground group-hover:text-foreground transition-colors">
                            <span className="text-indigo-500 mt-0.5 font-semibold">✓</span>
                            <span>{item}</span>
                          </li>
                        ))}
                      </ul>
